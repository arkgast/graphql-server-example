...
export const getPostIdsForUser = (userSource, args, context) => {
  ...
  if (after) {
    const [id, created_at] = after.split(':') // eslint-disable-line
    query = query
...
const getFrienshipLevels = (nodeId) => {
...
export const createPost = (body, level, context) => {
  ...
  const created_at = new Date().toISOString().split('T')[0] // eslint-disable-line
